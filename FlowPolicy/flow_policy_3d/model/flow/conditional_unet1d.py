...
class ConditionalResidualBlock1D(nn.Module):
    ...
    def forward(self, x, cond=None):
        ...
        '''
        out = self.blocks[0](x)#B,28,4--> B,512,4 --> B,1024,4 -->B,2048,4 --> B,1024,4 --> B,512,4
        if cond is not None:
...
class ConditionalUnet1D(nn.Module):
    def __init__(self, 
        ...
        in_out = list(zip(all_dims[:-1], all_dims[1:]))#[(28, 512), (512, 1024), (1024, 2048)]
        ...
        # self.condition_to_action = nn.Sequential(
        # nn.Linear(cond_dim, down_dims[0]),
        # )
    ...
    def forward(self, 
        ...
        """
        sample = einops.rearrange(sample, 'b h t -> b t h')#B,4,28->B,28,4
        # 1. time
        timesteps = timestep
        ...
        # cond_emb = self.condition_to_action(global_feature)#B,512
        ...
        for idx, (resnet, resnet2, downsample) in enumerate(self.down_modules):
            if self.use_down_condition:
                x = resnet(x, global_feature)#B,512,4     B,1024,2       B,2048,1
                if idx == 0 and len(h_local) > 0:
                    x = x + h_local[0]
                x = resnet2(x, global_feature)#B,512,4     B,1024,2       B,2048,1
                # if idx == 0 :
                #     x = torch.cat([cond_emb.unsqueeze(-1), x], dim=-1)#B,512,5
            else:
                x = resnet(x)
                if idx == 0 and len(h_local) > 0:
                    x = x + h_local[0]
                x = resnet2(x)
            h.append(x)
            x = downsample(x)#B,512,2        B,1024,1       B,2048,1


        for mid_module in self.mid_modules:
            if self.use_mid_condition:
                x = mid_module(x, global_feature)#B,2048,1    B,2048,1
            else:
                x = mid_module(x)
        ...
        for idx, (resnet, resnet2, upsample) in enumerate(self.up_modules):
            ...
            if self.use_up_condition:
                x = resnet(x, global_feature)#B,1024,1    B,512,2
                if idx == len(self.up_modules) and len(h_local) > 0:
                    x = x + h_local[1]
                x = resnet2(x, global_feature)#B,1024,1    B,512,2
            else:
                x = resnet(x)
                if idx == len(self.up_modules) and len(h_local) > 0:
                    x = x + h_local[1]
                x = resnet2(x)
            x = upsample(x)#B,1024,2    B,512,4


        x = self.final_conv(x)#B,28,4
